        ...
        <Svg width={GRAPH_WIDTH} height={GRAPH_HEIGHT}>
          ...
          {points.map((point, i) => (
            <React.Fragment key={i}>
              {point.value > 0 && <SvgText x={point.x} y={point.y - 8} fontSize={10} fill={theme.text} textAnchor="middle">{formatValue(point.value)}</SvgText>}
              <Circle cx={point.x} cy={point.value > 0 ? point.y : padding.top + graphH} r={point.value > 0 ? 6 : 3} fill={point.value > 0 ? theme.primary : theme.border} />
